...
//
// This function PANICS if options is not an instance of [*psiphon.Config].
...
func (tl *targetLoader) Load(ctx context.Context) ([]model.ExperimentTarget, error) {
	...
	// single target with an empty URL so the experiment runs once against the
	// default URL through the psiphon tunnel.
	if len(targets) <= 0 {
		targets = append(targets, &Target{
			Config: tl.options,
			URL:    "",
