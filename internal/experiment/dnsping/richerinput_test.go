...
func TestTarget(t *testing.T) {
	...
	t.Run("Input", func(t *testing.T) {
		if target.Input() != "udp://8.8.8.8:53" {
			t.Fatal("invalid Input")
		}
	})

	t.Run("Options", func(t *testing.T) {
		expect := []string{"Domains=example.com", "Repetitions=4"}
	...
	t.Run("String", func(t *testing.T) {
		if target.String() != "udp://8.8.8.8:53" {
	...
	cases := []testcase{
		{
			...
			loader: &targetloading.Loader{
				ExperimentName: "dnsping",
				InputPolicy:    model.InputOrStaticDefault,
				Logger:         model.DiscardLogger,
				Session:        &mocks.Session{},
				StaticInputs:   []string{"udp://8.8.8.8:53"},
			},
			expectErr: nil,
			expectTargets: []model.ExperimentTarget{
				&Target{
...

		{
			...
			loader: &targetloading.Loader{
				...
				StaticInputsConfig: []json.RawMessage{
					json.RawMessage(`{"domains":"ooni.org"}`),
					json.RawMessage(`{}`),
				},
			},
			expectErr: nil,
			expectTargets: []model.ExperimentTarget{
				&Target{
					URL:    "udp://8.8.8.8:53",
					Config: &Config{Repetitions: 3, Domains: "ooni.org"}, // overridden per-input
				},
				&Target{
					URL:    "udp://1.1.1.1:53",
		...
		{
			name:    "no input errors because dnsping has no static default resolver",
			options: &Config{},
			loader: &targetloading.Loader{
				ExperimentName: "dnsping",
				InputPolicy:    model.InputOrStaticDefault,
