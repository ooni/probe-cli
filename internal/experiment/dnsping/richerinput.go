...
//
// This function PANICS if options is not an instance of [*dnsping.Config].
...
func (tl *targetLoader) Load(ctx context.Context) ([]model.ExperimentTarget, error) {
	...
	// This experiment has no static default resolver list, so it requires input:
	// the input is the resolver endpoint (e.g. udp://8.8.8.8:53) to ping.
	if len(targets) <= 0 {
