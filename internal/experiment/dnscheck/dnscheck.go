...
type richerInputTarget struct {
	// Config contains the experiment measurer options.
	Config Config `json:"config"`

	// URL is the target URL to measure.
...
func (r richerInputTarget) Options() []string {
	return richerinput.OptionsToStringList(r.Config)
}

// Input implements [model.RicherInputTarget].
func (r richerInputTarget) Input() model.MeasurementInput {
	return model.MeasurementInput(r.URL)
}

// richerInputLoader loads richer input for this experiment.
type richerInputLoader struct{}

// defaultRicherInput contains the default richer input for this experiment.
//
// TODO(bassosimone): this list should be obviously extended a lot.
var defaultRicherInput = []richerInputTarget{
	{
		Config: Config{
			HTTP3Enabled: false,
		},
...
func (v *richerInputLoader) Load(ctx context.Context, config *model.RicherInputConfig) ([]richerInputTarget, error) {
	...
	// Load the ExtraOptions field.
	options, err := richerinput.LoadOptions[Config](config)
	if err != nil {
		return nil, err
	}
...
func (v *richerInputLoader) callRicherInputAPI(_ context.Context, _ *model.RicherInputConfig) ([]richerInputTarget, error) {
	// TODO(bassosimone): this should become a probeservices API call to fetch richer
	// input for this experiment possibly along with caching support.
	return defaultRicherInput, nil
}
...
func (vm *richerInputMeasurer) Run(ctx context.Context, args *richerinput.MeasurerRunArgs[richerInputTarget]) error {
	// TODO(bassosimone): I see this code evolving as follows:
	//
	// 1. the [NewExperimentMeasurer] constructor will become a private implementation detail;
	//
	// 2. once that is done, the code should probably read the input (e.g., the URL for webconnectivity)
	// from the richer input target rather than from the measurement's Input.
	//
	// In any case, these changes will only be possible when we're only using richer input to
	// execute experiments, sich that the old code has been removed.
	mx := NewExperimentMeasurer(args.Target.Config)
	return mx.Run(ctx, &model.ExperimentArgs{
