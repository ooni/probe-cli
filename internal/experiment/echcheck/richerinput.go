...
//
// This function PANICS if options is not an instance of [*echcheck.Config].
...
func (tl *targetLoader) Load(ctx context.Context) ([]model.ExperimentTarget, error) {
	...
	// This experiment's input is OPTIONAL: when there's no input we return a
	// single target with an empty URL so the experiment runs once with the
	// experiment-wide config and the default URL.
	if len(targets) <= 0 {
		targets = append(targets, &Target{
			Config: tl.options,
			URL:    "",
