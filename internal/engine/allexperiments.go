...
import (
	...
	"github.com/ooni/probe-cli/v3/internal/engine/experiment/download"
...
var experimentsByName = map[string]func(*Session) *ExperimentBuilder{
	...
	"download": func(session *Session) *ExperimentBuilder {
		return &ExperimentBuilder{
			build: func(config interface{}) *Experiment {
				return NewExperiment(session, download.NewExperimentMeasurer(
