...
var PortFilteringDefaultInput = []string{
	...
	"8080",  // tcp, udp - random ports to detect IP blocking
	"5050",  // tcp, udp - random ports to detect IP blocking
	"5000",  // tcp, udp - random ports to detect IP blocking
}
...
func StaticBareInputForExperiment(name string) ([]string, error) {
	...
	case "portfiltering":
		rand.Shuffle(len(PortFilteringDefaultInput), func(i, j int) {
			PortFilteringDefaultInput[i], PortFilteringDefaultInput[j] = PortFilteringDefaultInput[j], PortFilteringDefaultInput[i]
		})
		return PortFilteringDefaultInput, nil
	default:
