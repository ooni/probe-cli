...
func (d *Descriptor) genTestTheDatabaseIsReplaced(sb *strings.Builder) {
	...
	fmt.Fprint(sb, "\t// step 2: we disable register and login. We\n")
	fmt.Fprint(sb, "\t// should be okay because of the token but the server\n")
	fmt.Fprint(sb, "\t// does not recognize us, so we relogin again\n")
	fmt.Fprint(sb, "\tapi.Err = ErrUnauthorized\n")
	fmt.Fprint(sb, "\tapi.Response = nil\n")
	...
	fmt.Fprint(sb, "\tif loginAPI.CountCall != 2 {\n")
	fmt.Fprint(sb, "\t\tt.Fatal(\"invalid loginAPI.CountCall\")\n")
	fmt.Fprint(sb, "\t}\n")

	fmt.Fprint(sb, "\tif registerAPI.CountCall != 2 {\n")
	fmt.Fprint(sb, "\t\tt.Fatal(\"invalid registerAPI.CountCall\")\n")
	fmt.Fprint(sb, "\t}\n")
