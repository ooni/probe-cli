...
Each result contains one or more _measurements_, which are the JSON
files produced by an experiment.

What's more, some experiments produce as ouput a single measurement, while
others (the ones that take _inputs_) produce one or more.

For example, a network performance experiment such as DASH always
produce a single measurement describing the measured performance. The
Web Connectivity test, instead, produces a measurement for every URL
that has been passed to it as _input_.
...
provides information on the outcome of each experiment
within a specific result `<id>`.
...
```JavaScript
// entry describing a measurement (an experiment for all groups but
// the websites group, where it describes the measurement of a single
// URL with an input list of URLs)
{
  "fields": {
    "asn": 30722,
    "failure_msg": "",
